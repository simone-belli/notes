...
1. **Write the detailed explanation to `_explain.md`** at the repo root — more depth than the permanent file, including intuition and context. Do **not** run `open` (or any other command) on it — never launch a GUI application to display the file; the user opens it themselves. This file is gitignored and never committed. If the query says `BRIEFLY`, keep it concise.
