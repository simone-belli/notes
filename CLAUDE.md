...
Every entry links to a child page (or subdirectory) by its **title**, not its filename, and the icon encodes the type: `:material-text-box-outline:` for a `note`, `:material-card-bulleted-outline:` for a `ref`. Subdirectories instead get a **topic-specific** Material icon that evokes what they hold (`:material-test-tube:` for Testing, `:material-bookshelf:` for Stdlib, `:material-table:` for Pandas) — this applies at every level, not just the section hubs. Verify the name exists in the installed theme's `.icons/material/` directory before using it; fall back to `:material-folder-outline:` only when nothing fits. The title is the child's H1; for a subdirectory use the short last segment of its breadcrumb H1 (e.g. `Python — Language / Concurrency` → **Concurrency**), and trim any leading breadcrumb prefix that just repeats the current page's context (on the Pandas page, `Pandas — Iteration` → **Iteration**). The link target is still the file/directory path.
