...
`pydantic-settings` extends [Pydantic](pydantic.md) with `BaseSettings`: a model that reads field values from [environment variables](../../../tools/shell/env-vars.md) and `.env` files automatically. Use it for all configuration — API keys, file paths, feature flags, timeouts.
...
**[FastAPI](../fastapi/dependencies.md) pattern** — override the settings dependency instead of patching (the same `Settings` also backs a [`Depends`-based API-key check](../fastapi/dependencies.md#api-key-check-as-a-dependency-a-toy-guard)):
```python
# dependencies.py
from functools import lru_cache
