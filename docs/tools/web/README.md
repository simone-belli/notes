# Tools / Web

Making HTTP requests and serving web apps.

:material-card-bulleted-outline: **[curl](curl.md){ .lvl-intermediate }**
:   Command-line HTTP client: request parts as flags, sending data, auth, exit codes

:material-text-box-outline: **[HTTP Requests](http-request.md){ .lvl-basic }**
:   Anatomy of an HTTP request: request line, methods, headers, query strings, body formats

:material-card-bulleted-outline: **[HTTP Status Codes](http-status-codes.md){ .lvl-basic }**
:   HTTP status codes: the five classes, common codes, 401 vs 403, retry rules

:material-text-box-outline: **[Render — deploying a web service](render.md){ .lvl-advanced }**
:   Deploy a Python web app to Render: build/start commands, `$PORT`, env vars, staged post-deploy verification, free-tier realities
