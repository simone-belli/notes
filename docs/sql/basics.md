...
!!! warning "The fan-out trap"
    ...
    every extra item row. Fix: aggregate the many-side down to one row per
    key *before* joining, not after. This is the same trap as a one-to-many
    `df.merge()` inflating a `.groupby().sum()` run afterward in pandas —
    check join-key cardinality before trusting a post-join aggregate.
