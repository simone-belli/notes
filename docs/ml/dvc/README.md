...
:   `dvc.yaml` stages as a dependency graph, `dvc repro`'s hash-based staleness check, `dvc.lock`, and metric/param diffs

:material-card-bulleted-outline: **[Versioning](versioning.md)**
:   Where `dvc init` goes, pointer files, the content-addressed cache and its link modes, auto-managed `.gitignore`, and the Git ↔ DVC command mapping
