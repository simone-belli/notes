...
:   Where `dvc init` goes, pointer files, the content-addressed cache and its link modes, auto-managed `.gitignore`, reading a dataset's version (hash + revision) from the shell or `dvc.api`, the committed vs. local config layers, and the Git ↔ DVC command mapping
